...
interface Props {
  imovel: Pick<Imovel, 'disponivel_para' | 'preco_venda' | 'preco_locacao' | 'iptu'>
  compact?: boolean
}

export default function PriceDisplay({ imovel, compact = false }: Props) {
  const { disponivel_para, preco_venda, preco_locacao, iptu } = imovel
    ...
    <div className="space-y-4">
      ...
      {(disponivel_para === 'locacao' || disponivel_para === 'ambos') && (
        <div className="bg-slate-50 rounded-lg p-4">
          ...
          {preco_locacao ? (
            <>
              <p className="text-2xl font-bold text-slate-900">{formatCurrency(preco_locacao)}<span className="text-base font-normal text-slate-500">/mês</span></p>
